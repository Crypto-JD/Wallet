...
getNativeBalances = async () => {
  ...
  const bscBalance = await Moralis.Web3API.account.getNativeBalance({
    chain: "bsc"
  });
...
getERC20Balances = async () => {
  ...
  let bscTokens = await Moralis.Web3API.account.getTokenBalances({
    chain: 'bsc'
  });
...
getStats = async () => {
  ...
  // LIQUIDITY
  let pairReserves = await Moralis.Web3API.defi.getPairReserves({
    chain: 'bsc',
    pair_address: "0x20F6Ca2445695CC2d3A2137F1823E7929581c699"
  });
